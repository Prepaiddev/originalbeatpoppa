...
export async function GET(req: NextRequest, { params }: { params: Promise<{ orderId: string }> }) {
  try {
    ...
    const downloadLinks = await Promise.all(items.map(async (item: any) => {
      ...
      const path = beat.master_url || beat.audio_url; // Fallback to preview if master not available
      
      // Extract path from storage URL if needed, or assume it's just the filename/path in bucket
      const bucket = beat.master_url ? 'masters' : 'beats';
      
      const { data, error } = await supabase.storage
        .from(bucket)
        .createSignedUrl(path, 172800); // 48 hours
